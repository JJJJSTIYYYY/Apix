...
const api = {
  ...
  openFileDialog: (type, extensions = [], title = "APIX") => electron.ipcRenderer.invoke("openFileDialog", type, extensions, title),
  ...
  createCronTask: (cid, cron_meta) => electron.ipcRenderer.invoke("api:create_cron_task", cid, cron_meta),
  getCronTaskList: (cid) => electron.ipcRenderer.invoke("api:get_cron_task_list", cid),
  updateCronTask: (tid, repeat, exec_time, new_info) => electron.ipcRenderer.invoke("api:update_cron_task", tid, repeat, exec_time, new_info),
  // Config AI
