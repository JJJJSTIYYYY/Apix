...
function buildRenderItems(input?: string | MessageChunk[]): RenderItem[] {
...
const thinkRenderItems = computed(() => buildRenderItems(props.msg.think))
const contentRenderItems = computed(() => buildRenderItems(props.msg.content))
...
watch(
  () => props.msg.pending,
  async () => {
    if (!props.msg.pending) {
      console.log('Message not pending')
      const lastThink = thinkRenderItems.value.at(-1)
      if (lastThink?.kind === 'text') {
        if (lastThink.html.includes('[Conversation Abort]')) {
          lastThink.html = lastThink.html.replace(
            '[Conversation Abort]',
            abortLabel
          )
          console.log('Replace [Conversation Abort] to label in think chunk.')
        }
      }
      const lastContent = contentRenderItems.value.at(-1)
      if (lastContent?.kind === 'text') {
        if (lastContent.html.includes('[Conversation Abort]')) {
          lastContent.html = lastContent.html.replace(
            '[Conversation Abort]',
            abortLabel
          )
          console.log('Replace [Conversation Abort] to label in content chunk.')
        }
      }
    }
  },
