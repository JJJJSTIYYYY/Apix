    ...
    <el-main class="main-area">
      <div class="ai-page-wrapper">
        ...
        <div class="chat-wrapper">
          <div class="work-dir-label" :class="{no_work_dir: show_work_dir===''}">{{ show_work_dir===''?'未指定工作目录，继续处理文件相关工作时请先关联项目':show_work_dir }}</div>
  ...
  for (const r of raw) {
    ...
    if (r.role === 'ai' || r.role === 'info') {
      ...
      if (index === -1) {
        ...
        msg.content = [msg.content, r.content].filter(Boolean).join('\n\n')
        msg.think = [msg.think, r.think].filter(Boolean).join('\n\n')
...
function handleThinkChunkRtn(payload: any, historyId: string) {
  ...
  if (list[index].pending === true) {
    list[index].label = '思考中...'
    list[index].think = (list[index].think ?? '') + deltaThink
  }
}
...
function handleContentChunkRtn(payload: any, historyId: string) {
  ...
  if (list[index].pending === true) {
    list[index].label = '回答中...'
    list[index].content = (list[index].content ?? '') + deltaContent
  }
}
...
async function handleStreamAbort(payload: any, historyId: string) {
  ...
  if (index !== -1 && list[index].pending === true) {
    list[index].pending = false
    list[index].content = `${list[index].content ?? ''} [${reason}]`
...
function handleToolsChunkRtn(payload: any, historyId: string) {
  ...
  if (toolChunkRtn === 'search_keywords') {
    handleSearchKeywordsChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'search_urls') {
    handleSearchUrlsChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'configure_sandbox') {
    handleConfigureSandboxChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'load_skill') {
    handleLoadSkillChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'read_files') {
    handleReadFilesChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'write_file') {
    handleWriteFileChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'run_command') {
    handleRunCommandChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'run_code') {
    handleRunCodeChunkRtn(generationId, toolData, historyId)
    return
  }
  if (toolChunkRtn === 'ocr_analysis') {
    handleOcrAnalysisChunkRtn(generationId, toolData, historyId)
    return
  }
...
function handleSearchKeywordsChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '正在搜索关键词'
}

function handleSearchUrlsChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '正在获取网页内容'
}

function handleConfigureSandboxChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '配置沙箱环境'
}

function handleLoadSkillChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '加载技能包'
}

function handleReadFilesChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '读取文件'
}

function handleWriteFileChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '写入文件'
}

function handleRunCommandChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '执行命令'
}

function handleRunCodeChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '运行脚本'
}

function handleOcrAnalysisChunkRtn(generationId: string, data: any, historyId: string) {
  if (!data || !generationId) return
  const state = ensureGeneratingState(historyId)
  state.streamStateText = '正在分析'
}
...
async function sendMessage() {
  if (store.current_history_id === '-1') return
