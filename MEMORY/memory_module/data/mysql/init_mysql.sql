SET time_zone = '+08:00';

-- [id, user_uid, username, password, created_at]
DROP TABLE IF EXISTS users;
CREATE TABLE users (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    user_uid VARCHAR(64) NOT NULL UNIQUE COMMENT 'External user identifier',

    username VARCHAR(64) NOT NULL UNIQUE,

    password VARCHAR(64) NOT NULL,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) DEFAULT CHARSET=utf8mb4 COLLATE = utf8mb4_unicode_ci;



-- [id, user_uid, conversation_uid, title, session_id, last_active_at, latest_cursor, is_pinned, is_deleted, created_at]
DROP TABLE IF EXISTS conversations;
CREATE TABLE conversations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT 'Conversation id used in system',

    user_uid VARCHAR(64) NOT NULL COMMENT 'Owner user uid',

    conversation_uid VARCHAR(64) NOT NULL COMMENT 'Conversation uid that exposes to user',
    title VARCHAR(255) NOT NULL DEFAULT '新的聊天...' COMMENT 'Conversation title',

    session_id VARCHAR(64) DEFAULT '' COMMENT 'Tab identifier',

    last_active_at TIMESTAMP NOT NULL COMMENT 'Last message time',
    latest_cursor BIGINT NOT NULL DEFAULT 0 COMMENT 'Latest message id, Monotonic message cursor for this conversation',

    is_pinned BOOLEAN DEFAULT FALSE,
    is_deleted BOOLEAN DEFAULT FALSE,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    latest_timestamp BIGINT NOT NULL DEFAULT 0,

    work_dir VARCHAR(255) COMMENT 'Agent work dir',

    UNIQUE KEY uk_user_conversation (user_uid, conversation_uid),

    INDEX idx_user_latest_pinned (user_uid, is_pinned DESC, last_active_at DESC),

    INDEX idx_user_conversation_active (user_uid, conversation_uid, is_deleted),

    CONSTRAINT fk_conversation_user_uid
        FOREIGN KEY (user_uid)
        REFERENCES users(user_uid)
        ON DELETE CASCADE
) DEFAULT CHARSET=utf8mb4 COLLATE = utf8mb4_unicode_ci;



-- [id, memory_id, user_uid, memory_type, content, confidence, status, deprecated_reason, created_timestamp, updated_timestamp, deprecated_at]
DROP TABLE IF EXISTS longterm_memory;
CREATE TABLE longterm_memory (
    -- Identity
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT 'Internal auto id',
    memory_id CHAR(36) NOT NULL UNIQUE COMMENT 'ULID / external memory id',
    user_uid VARCHAR(64) NOT NULL COMMENT 'External user identifier',

    -- Memory core
    memory_type ENUM(
        'preference',
        'fact',
        'rule',
        'event',
        'transient'
    ) NOT NULL COMMENT 'Memory semantic type',
    content TEXT NOT NULL COMMENT 'Memory content',

    -- Model inputs
    confidence FLOAT NOT NULL
        CHECK (confidence >= 0 AND confidence <= 1),
    worth FLOAT NOT NULL
        CHECK (worth >= 0 AND worth <= 1),
    used_count INT UNSIGNED NOT NULL DEFAULT 0,

    -- Lifecycle
    status ENUM('active', 'deprecated') NOT NULL DEFAULT 'active',
    deprecated_reason TEXT DEFAULT NULL,
    deprecated_at BIGINT DEFAULT NULL COMMENT 'Unix timestamp * 1_000_000',

    -- Time (all microsecond-aligned)
    created_timestamp BIGINT NOT NULL COMMENT 'Unix timestamp * 1_000_000',
    updated_timestamp BIGINT NOT NULL COMMENT 'Unix timestamp * 1_000_000',

    -- Ranking
    score FLOAT NOT NULL DEFAULT 0 COMMENT 'Normalized ranking score [0,1]',

    -- Indexes
    UNIQUE INDEX ux_user_memory_active (
        user_uid,
        memory_id,
        status
    ),
    INDEX idx_user_active_score (
        user_uid,
        status,
        score DESC
    ),
    INDEX idx_gc_cold_start (
        status,
        used_count,
        created_timestamp
    ),

    -- FK
    CONSTRAINT fk_longterm_memory_user
        FOREIGN KEY (user_uid)
        REFERENCES users(user_uid)
        ON DELETE CASCADE
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



-- [id, user_uid, conversation_id, conversation_uid, role, content, extra, created_at, msg_cursor]
DROP TABLE IF EXISTS messages;
CREATE TABLE messages (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    user_uid VARCHAR(64) NOT NULL COMMENT 'Owner user uid',
    conversation_id BIGINT NOT NULL COMMENT 'id in table conversations, to make table joins more efficient.',

    conversation_uid VARCHAR(64) NOT NULL COMMENT 'The conversation uid that exposes to user, it will be used to query messages.',
    generation_id VARCHAR(64) NOT NULL COMMENT 'Generation id to identify different llm\'s generation task.',

    role ENUM('human', 'ai', 'system', 'tools', 'info') NOT NULL,
    content LONGTEXT,
    think LONGTEXT,

    extra JSON DEFAULT NULL COMMENT 'Tool args / metadata / ...',
    info JSON DEFAULT NULL COMMENT 'model / provider / duration / tokens / id (lang graph)',

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    msg_cursor BIGINT NOT NULL COMMENT 'Message cursor in one conversation',
    msg_timestamp BIGINT NOT NULL DEFAULT 0,

    INDEX idx_user_conversation_cursor (
        user_uid,
        conversation_uid,
        msg_cursor
    ),

    INDEX idx_conversation_created (conversation_id, created_at DESC),

    CONSTRAINT fk_message_user_uid
        FOREIGN KEY (user_uid)
        REFERENCES users(user_uid)
        ON DELETE CASCADE,

    CONSTRAINT fk_message_conversation
        FOREIGN KEY (conversation_id)
        REFERENCES conversations(id)
        ON DELETE CASCADE,

    UNIQUE KEY uk_conversation_cursor (conversation_id, msg_cursor)
) DEFAULT CHARSET=utf8mb4 COLLATE = utf8mb4_unicode_ci;



-- [id, file_id, file_name, file_path, file_size, mime_type, user_uid, deleted, upload_at, deleted_at]
DROP TABLE IF EXISTS file_store;
CREATE TABLE file_store (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    file_id VARCHAR(64) NOT NULL COMMENT 'Opaque file id generated by file_service',
    file_name VARCHAR(255) NOT NULL COMMENT 'Original file name from user',
    file_path VARCHAR(512) NOT NULL COMMENT 'Physical path in file_service',
    file_size BIGINT NOT NULL COMMENT 'Physical file size in file_service (bytes)',

    mime_type VARCHAR(128) DEFAULT 'unknow' COMMENT 'File mime type',

    user_uid VARCHAR(64) NOT NULL COMMENT 'Owner user uid',

    sha256 CHAR(64) COMMENT 'File hash',

    deleted BOOLEAN DEFAULT FALSE COMMENT 'Soft delete flag',
    upload_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT 'Upload time',
    deleted_at DATETIME DEFAULT NULL COMMENT 'Soft delete time',

    UNIQUE KEY uk_file_id (file_id),
    INDEX idx_user_upload (user_uid, upload_at),
    INDEX idx_user_file_id (user_uid, file_id),
    INDEX idx_user_active (user_uid, deleted, upload_at),
    INDEX idx_sha256 (sha256),

    CONSTRAINT fk_file_user_uid
        FOREIGN KEY (user_uid)
        REFERENCES users(user_uid)
        ON DELETE CASCADE
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



-- [id, file_id, file_name, file_path, file_size, mime_type, user_uid, deleted, upload_at, deleted_at]
DROP TABLE IF EXISTS agent_skills;
CREATE TABLE agent_skills (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    skill_id VARCHAR(64) NOT NULL COMMENT 'Skill id generated by file_service',
    skill_name VARCHAR(255) NOT NULL COMMENT 'User display name',
    skill_description TEXT NOT NULL COMMENT 'Skill description',
    skill_version VARCHAR(16) NOT NULL DEFAULT 'v1.0',

    package_path VARCHAR(512) NOT NULL COMMENT 'Physical path in file_service',
    package_size BIGINT NOT NULL COMMENT 'Skill package size in bytes',
    package_sha256 CHAR(64) DEFAULT NULL COMMENT 'Optional file hash',

    user_uid VARCHAR(64) NOT NULL COMMENT 'Owner user uid',

    is_active BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'If the skill is active',
    deleted BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'Soft delete flag',
    upload_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Upload time',
    deleted_at DATETIME DEFAULT NULL COMMENT 'Soft delete time',

    UNIQUE KEY uk_skill_id (skill_id),

    INDEX idx_user_skill_id (user_uid, skill_id),
    INDEX idx_user_upload (user_uid, upload_at DESC),
    INDEX idx_user_active (user_uid, deleted, upload_at DESC),

    CONSTRAINT fk_skill_user_uid
        FOREIGN KEY (user_uid)
        REFERENCES users(user_uid)
        ON DELETE CASCADE
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



-- [id, task_id, user_uid, conversation_uid, status, payload, result, created_at, finished_at]
DROP TABLE IF EXISTS task_info;
CREATE TABLE task_info (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    task_id VARCHAR(64) UNIQUE NOT NULL,
    user_uid VARCHAR(64) NOT NULL,
    conversation_uid VARCHAR(64) DEFAULT '',

    status ENUM('pending', 'running', 'done', 'failed', 'manual stopped') NOT NULL,

    tool_association VARCHAR(64) NOT NULL DEFAULT 'Unsigned',
    payload JSON NOT NULL,
    result JSON NOT NULL COMMENT 'result list',

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    finished_at DATETIME COMMENT 'Task finished time, explicitly set by application',

    UNIQUE KEY uk_task_id (task_id)

) DEFAULT CHARSET=utf8mb4 COLLATE = utf8mb4_unicode_ci;



-- [id, tool_name, description, category, version_tag, idempotent, timeout_seconds, is_enabled, upload_user, created_at, updated_at]
DROP TABLE IF EXISTS tools_registry;
CREATE TABLE tools_registry (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    tool_name VARCHAR(64) NOT NULL UNIQUE COMMENT 'Logical tool name, exposed to AI',
    description TEXT NOT NULL COMMENT 'Human-readable description for LLM',

    category VARCHAR(32) NOT NULL DEFAULT 'general',
    version_tag VARCHAR(32) NOT NULL DEFAULT 'v1.0.0',
    operating_system ENUM(
        'win',
        'linux',
        'macos'
    ) NOT NULL,

    idempotent BOOLEAN DEFAULT FALSE COMMENT 'Whether the tool output are not association with call times',
    timeout_seconds INT NOT NULL DEFAULT 300,

    is_enabled BOOLEAN DEFAULT TRUE COMMENT 'Soft disable without deleting',

    upload_user VARCHAR(64) NOT NULL DEFAULT 'Unsigned',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    INDEX idx_category_enabled (category, is_enabled),

    CONSTRAINT fk_file_user_uid
        FOREIGN KEY (upload_user)
        REFERENCES users(user_uid)
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



-- [id, tool_name, description, category, version_tag, idempotent, timeout_seconds, is_enabled, upload_user, created_at, updated_at]
DROP TABLE IF EXISTS tools_args_schema;
CREATE TABLE tools_args_schema (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    tool_name VARCHAR(64) NOT NULL,

    param_name VARCHAR(64) NOT NULL,
    param_type ENUM(
        'string',
        'integer',
        'number',
        'boolean',
        'array',
        'object'
    ) NOT NULL,

    description TEXT DEFAULT '',
    is_required BOOLEAN DEFAULT TRUE,
    default_value TEXT DEFAULT NULL,

    UNIQUE KEY uk_tool_param (tool_name, param_name),

    CONSTRAINT fk_args_tool_name
        FOREIGN KEY (tool_name)
        REFERENCES tools_registry(tool_name)
        ON DELETE CASCADE,

    CHECK (
        (is_required = TRUE) OR
        (is_required = FALSE AND default_value IS NOT NULL)
    )
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



-- Runtime execution binding (used by Tools service only)
DROP TABLE IF EXISTS tools_runtime;
CREATE TABLE tools_runtime (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,

    tool_name VARCHAR(64) NOT NULL,

    script_path VARCHAR(255) NOT NULL COMMENT 'Absolute or container path',
    entrypoint VARCHAR(128) NOT NULL COMMENT 'Function / class / CLI entry',

    runtime ENUM('python', 'bash', 'node', 'binary') NOT NULL DEFAULT 'python',

    working_dir VARCHAR(255) DEFAULT '',
    env_vars JSON DEFAULT NULL COMMENT 'Optional runtime env injection',

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,

    UNIQUE KEY uk_runtime_tool (tool_name),

    CONSTRAINT fk_runtime_tool_name
        FOREIGN KEY (tool_name)
        REFERENCES tools_registry(tool_name)
        ON DELETE CASCADE
) DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;



DELIMITER $$

CREATE PROCEDURE fetch_tool_runtime (
    IN p_tool_name VARCHAR(64)
)
BEGIN
    SELECT
        tool_name,
        script_path,
        entrypoint,
        runtime,
        working_dir,
        env_vars
    FROM tools_runtime
    WHERE tool_name = p_tool_name
    LIMIT 1;
END$$

DELIMITER ;



-- Stored Procedure: ensure_user_exists
DROP PROCEDURE IF EXISTS ensure_user_exists;
DELIMITER $$

CREATE PROCEDURE ensure_user_exists (
    IN p_user_uid VARCHAR(64)
)
BEGIN
    SELECT user_uid, username
    FROM users
    WHERE user_uid = p_user_uid;
END$$

DELIMITER ;



-- Stored Procedure: create_conversation
DROP PROCEDURE IF EXISTS create_conversation;
DELIMITER $$

CREATE PROCEDURE create_conversation (
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64),
    IN p_title VARCHAR(255),
    IN p_session_id VARCHAR(64)
)
BEGIN
    INSERT INTO conversations (
        user_uid,
        conversation_uid,
        title,
        session_id,
        last_active_at,
        latest_cursor
    )
    VALUES (
        p_user_uid,
        p_conversation_uid,
        COALESCE(p_title, '新的聊天...'),
        COALESCE(p_session_id, ''),
        CURRENT_TIMESTAMP,
        0
    );
END$$

DELIMITER ;



-- Stored Procedure: update_conversation
DROP PROCEDURE IF EXISTS update_conversation;
DELIMITER $$

CREATE PROCEDURE update_conversation (
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64),
    IN p_title VARCHAR(255),
    IN p_session_id VARCHAR(64),
    IN p_is_pinned TINYINT(1),
    IN p_is_deleted TINYINT(1)
)
BEGIN
    UPDATE conversations
    SET
        title = IF(p_title IS NULL, title, p_title),
        session_id = IF(p_session_id IS NULL, session_id, p_session_id),
        is_pinned = IF(p_is_pinned IS NULL, is_pinned, p_is_pinned),
        is_deleted = IF(p_is_deleted IS NULL, is_deleted, p_is_deleted)
    WHERE user_uid = p_user_uid
      AND conversation_uid = p_conversation_uid;

    -- Return affected rows so caller knows what happened
    SELECT ROW_COUNT() AS affected_rows;
END$$

DELIMITER ;



-- Stored Procedure: fetch_conversation_list
DROP PROCEDURE IF EXISTS fetch_conversation_list;
DELIMITER $$

CREATE PROCEDURE fetch_conversation_list (
    IN p_user_uid VARCHAR(64)
)
BEGIN
    SELECT
        conversation_uid,
        session_id,
        title,
        last_active_at,
        created_at,
        latest_cursor,
        is_pinned
    FROM conversations
    WHERE user_uid = p_user_uid
        AND is_deleted != TRUE
    ORDER BY is_pinned DESC, last_active_at DESC;
END$$

DELIMITER ;



-- Stored Procedure: insert_longterm_memory
DROP PROCEDURE IF EXISTS insert_longterm_memory;
DELIMITER $$

CREATE PROCEDURE insert_longterm_memory (
    IN p_memory_id CHAR(36),
    IN p_user_uid VARCHAR(64),
    IN p_memory_type VARCHAR(32),
    IN p_content TEXT,
    IN p_confidence FLOAT,
    IN p_worth FLOAT,
    IN p_timestamp BIGINT,
    IN p_score FLOAT
)
BEGIN
    INSERT INTO longterm_memory (
        memory_id,
        user_uid,
        memory_type,
        content,
        confidence,
        worth,
        used_count,
        status,
        created_timestamp,
        updated_timestamp,
        score
    ) VALUES (
        p_memory_id,
        p_user_uid,
        p_memory_type,
        p_content,
        p_confidence,
        p_worth,
        0,
        'active',
        p_timestamp,
        p_timestamp,
        p_score
    )
    ON DUPLICATE KEY UPDATE
        content = VALUES(content),
        confidence = VALUES(confidence),
        worth = VALUES(worth),
        memory_type = VALUES(memory_type),
        status = 'active',
        updated_timestamp = VALUES(updated_timestamp),
        score = VALUES(score);
END$$

DELIMITER ;



-- Stored Procedure: modify_longterm_memory
DROP PROCEDURE IF EXISTS modify_longterm_memory;
DELIMITER $$

CREATE PROCEDURE modify_longterm_memory (
    IN p_memory_id CHAR(36),
    IN p_user_uid VARCHAR(64),
    IN p_memory_type VARCHAR(32),
    IN p_new_content TEXT,
    IN p_new_confidence FLOAT,
    IN p_new_worth FLOAT,
    IN p_timestamp BIGINT,
    IN p_score FLOAT
)
BEGIN
    DECLARE v_exists INT;

    START TRANSACTION;

    SELECT 1 INTO v_exists
    FROM longterm_memory
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
      AND status = 'active'
    FOR UPDATE;

    IF v_exists IS NULL THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Active memory not found';
    END IF;

    UPDATE longterm_memory
    SET
        content = p_new_content,
        confidence = p_new_confidence,
        worth = p_new_worth,
        memory_type = p_memory_type,
        updated_timestamp = p_timestamp,
        score = p_score
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
      AND status = 'active';

    COMMIT;
END$$

DELIMITER ;



-- Stored Procedure: modify_longterm_memory
DROP PROCEDURE IF EXISTS refresh_longterm_memory;
DELIMITER $$

CREATE PROCEDURE refresh_longterm_memory (
    IN p_memory_id CHAR(36),
    IN p_user_uid VARCHAR(64),
    IN p_timestamp BIGINT,
    IN p_score FLOAT
)
BEGIN
    DECLARE v_exists INT;

    START TRANSACTION;

    SELECT 1 INTO v_exists
    FROM longterm_memory
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
      AND status = 'active'
    FOR UPDATE;

    IF v_exists IS NULL THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Active memory not found';
    END IF;

    UPDATE longterm_memory
    SET
        used_count = used_count + 1,
        updated_timestamp = p_timestamp,
        score = p_score
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
      AND status = 'active';

    COMMIT;
END$$

DELIMITER ;



-- Stored Procedure: deprecate_longterm_memory
DROP PROCEDURE IF EXISTS deprecate_longterm_memory;
DELIMITER $$

CREATE PROCEDURE deprecate_longterm_memory (
    IN p_memory_id CHAR(36),
    IN p_user_uid VARCHAR(64),
    IN p_confidence FLOAT,
    IN p_reason TEXT,
    IN p_timestamp BIGINT
)
BEGIN
    DECLARE v_exists INT;

    START TRANSACTION;

    SELECT 1 INTO v_exists
    FROM longterm_memory
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
      AND status = 'active'
    FOR UPDATE;

    IF v_exists IS NULL THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Active memory not found';
    END IF;

    UPDATE longterm_memory
    SET
        status = 'deprecated',
        confidence = p_confidence,
        deprecated_reason = p_reason,
        deprecated_at = p_timestamp
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
      AND status = 'active';

    COMMIT;
END$$

DELIMITER ;



-- Stored Procedure: fetch_longterm_memory
DROP PROCEDURE IF EXISTS fetch_longterm_memory;
DELIMITER $$

CREATE PROCEDURE fetch_longterm_memory (
    IN p_user_uid VARCHAR(64),
    IN p_limit INT
)
BEGIN
    SELECT
        memory_id,
        memory_type,
        content,
        confidence,
        worth,
        used_count,
        created_timestamp,
        updated_timestamp,
        score
    FROM longterm_memory
    WHERE user_uid = p_user_uid
      AND status = 'active'
    ORDER BY score DESC
    LIMIT p_limit;
END$$

DELIMITER ;



-- Stored Procedure: fetch_longterm_memory_for_calc_score
DROP PROCEDURE IF EXISTS fetch_longterm_memory_for_calc_score;
DELIMITER $$

CREATE PROCEDURE fetch_longterm_memory_for_calc_score (
    IN p_memory_id CHAR(36),
    IN p_user_uid VARCHAR(64)
)
BEGIN
    SELECT *
    FROM longterm_memory
    WHERE memory_id = p_memory_id
      AND user_uid = p_user_uid
    LIMIT 1;
END$$

DELIMITER ;



-- Stored Procedure: append_message
DROP PROCEDURE IF EXISTS append_message;
DELIMITER $$

CREATE PROCEDURE append_message (
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64),
    IN p_role ENUM('human', 'ai', 'system', 'tools', 'info'),
    IN p_content LONGTEXT,
    IN p_think LONGTEXT,
    IN p_extra JSON,
    IN p_info JSON,
    IN p_generation_id VARCHAR(64),
    IN p_msg_timestamp BIGINT
)
BEGIN
    DECLARE v_conversation_id BIGINT;
    DECLARE v_next_cursor BIGINT;
    DECLARE v_latest_timestamp BIGINT;
    DECLARE v_not_found INT DEFAULT 0;
    DECLARE v_message_id BIGINT;

    -- Handler: SELECT ... INTO returns NOT FOUND when no row matched
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_not_found = 1;

    START TRANSACTION;

    -- 1. Lock conversation row
    SELECT
        id,
        latest_cursor,
        latest_timestamp
    INTO
        v_conversation_id,
        v_next_cursor,
        v_latest_timestamp
    FROM conversations
    WHERE user_uid = p_user_uid
      AND conversation_uid = p_conversation_uid
      AND is_deleted = FALSE
    FOR UPDATE;

    IF v_not_found = 1 THEN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Conversation not found or deleted';
    END IF;

    -- 2. Advance cursor (monotonic)
    SET v_next_cursor = v_next_cursor + 1;

    -- 3. Insert message
    INSERT INTO messages (
        user_uid,
        conversation_id,
        conversation_uid,
        role,
        content,
        think,
        extra,
        info,
        msg_cursor,
        generation_id,
        msg_timestamp
    )
    VALUES (
        p_user_uid,
        v_conversation_id,
        p_conversation_uid,
        p_role,
        p_content,
        p_think,
        p_extra,
        p_info,
        v_next_cursor,
        p_generation_id,
        p_msg_timestamp
    );

    SET v_message_id = LAST_INSERT_ID();

    -- 4. Update conversation (no strict timestamp monotonicity)
    UPDATE conversations
    SET
        latest_cursor = v_next_cursor,
        latest_timestamp = GREATEST(
            IFNULL(latest_timestamp, p_msg_timestamp),
            p_msg_timestamp
        ),
        last_active_at = CURRENT_TIMESTAMP
    WHERE id = v_conversation_id;

    -- 5. Return result
    SELECT
        v_message_id AS msg_id,
        v_next_cursor AS msg_cursor,
        p_msg_timestamp AS msg_timestamp,
        NOW() AS created_at;

    COMMIT;
END$$

DELIMITER ;



-- Stored Procedure: fetch_messages_after_cursor
DROP PROCEDURE IF EXISTS fetch_messages_after_cursor;
DELIMITER $$

CREATE PROCEDURE fetch_messages_after_cursor (
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64),
    IN p_after_cursor BIGINT,
    IN p_limit INT
)
BEGIN
    SELECT
        role,
        content,
        think,
        extra,
        info,
        msg_cursor,
        created_at,
        generation_id
    FROM messages
    WHERE user_uid = p_user_uid
      AND conversation_uid = p_conversation_uid
      AND msg_cursor >= p_after_cursor
      AND role != 'info'
    ORDER BY msg_cursor ASC
    LIMIT p_limit;
END$$

DELIMITER ;



-- Stored Procedure: fetch_messages_after_cursor
DROP PROCEDURE IF EXISTS fetch_messages_for_user;
DELIMITER $$

CREATE PROCEDURE fetch_messages_for_user (
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64)
)
BEGIN
    SELECT
        role,
        content,
        think,
        extra,
        info,
        msg_cursor,
        generation_id,
        created_at
    FROM messages
    WHERE user_uid = p_user_uid
      AND conversation_uid = p_conversation_uid
      AND role IN ('ai', 'human', 'info')
    ORDER BY msg_cursor ASC;
END$$

DELIMITER ;



-- Stored Procedure: insert_file_info
DROP PROCEDURE IF EXISTS insert_file_info;
DELIMITER $$

CREATE PROCEDURE insert_file_info (
    IN p_file_id VARCHAR(64),
    IN p_file_name VARCHAR(255),
    IN p_file_path VARCHAR(512),
    IN p_file_size VARCHAR(32),
    IN p_mime_type VARCHAR(128),
    IN p_user_uid VARCHAR(64),
    IN p_sha256 CHAR(64)
)
BEGIN

    INSERT INTO file_store (
        file_id,
        file_name,
        file_path,
        file_size,
        mime_type,
        user_uid,
        deleted,
        upload_at,
        deleted_at,
        sha256
    )
    VALUES (
        p_file_id,
        p_file_name,
        p_file_path,
        p_file_size,
        p_mime_type,
        p_user_uid,
        FALSE,
        NOW(),
        NULL,
        p_sha256
    );

END $$

DELIMITER ;



DROP PROCEDURE IF EXISTS update_file_status;
DELIMITER $$

CREATE PROCEDURE update_file_status (
    IN p_file_id VARCHAR(64),
    IN p_user_uid VARCHAR(64),
    IN p_deleted BOOLEAN
)
BEGIN

    UPDATE file_store
    SET
        deleted = p_deleted,
        deleted_at = IF(p_deleted = TRUE, NOW(), NULL)
    WHERE
        file_id = p_file_id
        AND user_uid = p_user_uid
        AND deleted <> p_deleted;

END $$

DELIMITER ;



DROP PROCEDURE IF EXISTS fetch_recent_files;
DELIMITER $$

CREATE PROCEDURE fetch_recent_files (
    IN p_user_uid VARCHAR(64),
    IN p_limit INT
)
BEGIN

    SELECT
        file_id,
        file_name,
        file_path,
        upload_at,
        file_size,
        sha256
    FROM file_store
    WHERE
        user_uid = p_user_uid
        AND deleted = FALSE
        AND upload_at >= NOW() - INTERVAL 5 DAY
    ORDER BY upload_at DESC
    LIMIT p_limit;

END $$

DELIMITER ;



DROP PROCEDURE IF EXISTS fetch_target_file;
DELIMITER $$

CREATE PROCEDURE fetch_target_file (
    IN p_user_uid VARCHAR(64),
    IN p_file_id VARCHAR(64)
)
BEGIN

    SELECT
        file_id,
        file_name,
        file_path,
        upload_at,
        file_size,
        sha256
    FROM file_store
    WHERE
        user_uid = p_user_uid
        AND file_id = p_file_id
        AND deleted = FALSE
    LIMIT 1;

END$$

DELIMITER ;



DROP PROCEDURE IF EXISTS insert_agent_skill;
DELIMITER $$

CREATE PROCEDURE insert_agent_skill (
    IN p_skill_id VARCHAR(64),
    IN p_skill_name VARCHAR(255),
    IN p_skill_description TEXT,
    IN p_skill_version VARCHAR(16),
    IN p_package_path VARCHAR(512),
    IN p_package_size BIGINT,
    IN p_package_sha256 CHAR(64),
    IN p_user_uid VARCHAR(64)
)
BEGIN

    DECLARE skill_exists INT DEFAULT 0;

    SELECT COUNT(1)
    INTO skill_exists
    FROM agent_skills
    WHERE skill_id = p_skill_id;

    IF skill_exists > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Skill already exists';
    END IF;

    INSERT INTO agent_skills (
        skill_id,
        skill_name,
        skill_description,
        skill_version,
        package_path,
        package_size,
        package_sha256,
        user_uid,
        is_active
    )
    VALUES (
        p_skill_id,
        p_skill_name,
        p_skill_description,
        p_skill_version,
        p_package_path,
        p_package_size,
        p_package_sha256,
        p_user_uid,
        FALSE
    );

END$$

DELIMITER ;



DROP PROCEDURE IF EXISTS update_agent_skill;
DELIMITER $$

CREATE PROCEDURE update_agent_skill (
    IN p_skill_id VARCHAR(64),
    IN p_user_uid VARCHAR(64),
    IN p_is_active BOOLEAN,
    IN p_deleted BOOLEAN
)
BEGIN

    UPDATE agent_skills
    SET
        is_active = COALESCE(p_is_active, is_active),
        deleted = COALESCE(p_deleted, deleted),
        deleted_at = CASE
            WHEN p_deleted = TRUE THEN NOW()
            ELSE deleted_at
        END
    WHERE
        skill_id = p_skill_id
        AND user_uid = p_user_uid
        AND deleted = FALSE;

END$$

DELIMITER ;



DROP PROCEDURE IF EXISTS fetch_agent_skills;
DELIMITER $$

CREATE PROCEDURE fetch_agent_skills (
    IN p_user_uid VARCHAR(64),
    IN p_limit INT
)
BEGIN

    SELECT
        skill_id,
        skill_name,
        skill_description,
        skill_version,
        package_path,
        package_size,
        is_active,
        upload_at
    FROM agent_skills
    WHERE
        user_uid = p_user_uid
        AND deleted = FALSE
    ORDER BY upload_at DESC
    LIMIT p_limit;

END$$

DELIMITER ;



DROP PROCEDURE IF EXISTS fetch_target_skill;
DELIMITER $$

CREATE PROCEDURE fetch_target_skill (
    IN p_user_uid VARCHAR(64),
    IN p_skill_id VARCHAR(64)
)
BEGIN

    SELECT
        skill_id,
        skill_name,
        skill_description,
        skill_version,
        package_path,
        package_size,
        is_active,
        upload_at
    FROM agent_skills
    WHERE
        user_uid = p_user_uid
        AND skill_id = p_skill_id
        AND deleted = FALSE
    LIMIT 1;

END$$

DELIMITER ;



-- Stored Procedure: insert_task_result
DROP PROCEDURE IF EXISTS create_task_record;
DELIMITER $$

CREATE PROCEDURE create_task_record (
    IN p_task_id VARCHAR(64),
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64),
    IN p_payload JSON,
    IN p_created_at DATETIME
)
BEGIN
    INSERT INTO task_info (
        task_id,
        user_uid,
        conversation_uid,
        status,
        payload,
        result,
        created_at
    )
    VALUES (
        p_task_id,
        p_user_uid,
        p_conversation_uid,
        'pending',
        p_payload,
        JSON_ARRAY(),
        p_created_at
    );
END$$

DELIMITER ;



-- Stored Procedure: insert_task_result
DROP PROCEDURE IF EXISTS update_task_result;
DELIMITER $$

CREATE PROCEDURE update_task_result (
    IN p_task_id VARCHAR(64),
    IN p_status ENUM('pending', 'running', 'done', 'failed', 'manual stopped'),
    IN p_result JSON,
    IN p_finished_at DATETIME
)
BEGIN
    DECLARE v_current_status ENUM('pending', 'running', 'done', 'failed', 'manual stopped');
    DECLARE v_transition_allowed BOOLEAN DEFAULT TRUE;

    -- Fetch current status
    SELECT status
    INTO v_current_status
    FROM task_info
    WHERE task_id = p_task_id
    LIMIT 1;

    -- Task not found -> still hard fail (this is a real error)
    IF v_current_status IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Task not found';
    END IF;

    /*
      State machine rules:

      WAITING : pending
      ACTIVE  : running
      TERMINAL: done / failed / manual stopped
    */

    -- manual stopped is the final terminal, cannot transition anymore
    IF v_current_status = 'manual stopped' THEN
        SET v_transition_allowed = FALSE;
    END IF;

    -- Same-state transitions are not allowed
    IF v_current_status = 'pending' AND p_status = 'pending' THEN
        SET v_transition_allowed = FALSE;
    END IF;

    IF v_current_status = 'running' AND p_status = 'running' THEN
        SET v_transition_allowed = FALSE;
    END IF;

    -- Disallow backward transition: running -> pending
    IF v_current_status = 'running' AND p_status = 'pending' THEN
        SET v_transition_allowed = FALSE;
    END IF;

    -- Natural terminal states cannot transition between each other
    IF v_current_status IN ('done', 'failed')
       AND p_status IN ('done', 'failed')
       AND v_current_status <> p_status THEN
        SET v_transition_allowed = FALSE;
    END IF;

    -- manual stopped can override any state except itself
    -- (covered implicitly by the first rule)

    -- Perform update only if transition is allowed
    IF v_transition_allowed THEN
        UPDATE task_info
        SET
            status = p_status,
            result = IF(
                JSON_TYPE(result) = 'ARRAY',
                JSON_ARRAY_APPEND(result, '$', p_result),
                JSON_ARRAY(p_result)
            ),
            finished_at = CASE
                WHEN p_status IN ('done', 'failed', 'manual stopped')
                THEN p_finished_at
                ELSE finished_at
            END
        WHERE task_id = p_task_id;
    END IF;

    -- Always return current task state from database
    SELECT
        id,
        task_id,
        user_uid,
        conversation_uid,
        status,
        payload,
        result,
        finished_at
    FROM task_info
    WHERE task_id = p_task_id
    LIMIT 1;

END$$

DELIMITER ;



-- Stored Procedure: fetch_task_info
DROP PROCEDURE IF EXISTS fetch_task_info;
DELIMITER $$

CREATE PROCEDURE fetch_task_info (
    IN p_task_id VARCHAR(64)
)
BEGIN
    SELECT
        task_id,
        conversation_uid,
        status,
        payload,
        result,
        created_at,
        finished_at
    FROM task_info
    WHERE task_id = p_task_id
    LIMIT 1;
END$$

DELIMITER ;



DROP PROCEDURE IF EXISTS fetch_batches_task_info;
DELIMITER $$

CREATE PROCEDURE fetch_batches_task_info (
    IN p_user_uid VARCHAR(64),
    IN p_conversation_uid VARCHAR(64)
)
BEGIN
    SELECT
        task_id,
        conversation_uid,
        status,
        tool_association,
        payload,
        result,
        created_at,
        finished_at
    FROM task_info
    WHERE user_uid = p_user_uid
      AND conversation_uid = p_conversation_uid
    ORDER BY created_at ASC;

END$$

DELIMITER ;