...
from apix.config.base_config import MEMO_REDIS_URL, REDIS_POOL_SIZE, DEFAULT_EXPIRE_SECONDS
...
class RedisService:
    ...
    1. Memo Redis:
        Key: "memo:{user_id}:{conversation_id}" or "longterm:{user_id}"
    ...
    2. Task Redis:
        Key: "task:{task_hash}"
        Task_hash: Generated by hash(cilent_id + conversation_id + tool_id) in Tool Service.
    ...
    def _build_memo_key(self, payload: dict, prefix: str = 'memo') -> str:
        ...
        if not all([user_id, conversation_id]):
            raise KeyError("[RedisService][_build_memo_key] user_id / conversation_id required")
...
auto_init.register(redis_server)