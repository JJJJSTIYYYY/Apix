...
from apix.agent.prebuilt.node.tools.web_search.providers.base import BaseSearchProvider
...
class SearchProviderRouter:
    ...
    def _hash_cache_key(self, *, task_type: str, config: AgentConfigSchema) -> str:
        ...
        if task_type == "link":
            payload = {
                "task_type": "link",
                "provider": config.get("keyword_search_provider") or "",
                "api_key": config.get("keyword_search_api_key") or "",
            }
        elif task_type == "content":
            payload = {
                "task_type": "content",
                "provider": config.get("web_scraping_provider") or "",
                "api_key": config.get("web_scraping_api_key") or "",
            }
        else:
    ...
    async def _choose_provider(
        ...
        *,
        ...
        if task_type == "link":
            explicit_provider = config.get("keyword_search_provider") or ""
            api_key = config.get("keyword_search_api_key") or ""
        else:
            explicit_provider = config.get("web_scraping_provider") or ""
            api_key = config.get("web_scraping_api_key") or ""
